...
def write_in_file(file_name: str, data: list[dict], open_file_method: str = "w"):
    with open(file_name, open_file_method, encoding="utf-8") as file:
        json.dump(sorted(data, key= lambda x: (x['age'], x['name'])), file, indent= 2)
...
def find_from_file(file_name, **kwargs):
    kwargs = dict(filter(lambda x: x[0] in ['name', 'country', 'age'], kwargs.items()))
    data = read_from_file(file_name)
    result = []
    for line in data:
        if all(line[k] == v for k, v in kwargs.items()):
            result.append(line)
    return result


FILE_NAME = "task_49.json"
