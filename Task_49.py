...
def find_from_file(file_name,name = None, country = None, age = None):
    inputs = list(filter(lambda x: x ,[name, country, age]))
    # print(inputs)
    with open(file_name, "r", encoding="utf-8") as file:
    ...
    for line in lines:
        ...
        if all(i in list_line for i in inputs):
            # print(list_line)
            result.append(list_line)
    return result
    # return [line.rstrip().split(", ") for line in file if line.rstrip().split(", ")[1] == country]


FILE_NAME = "task_49.txt"
